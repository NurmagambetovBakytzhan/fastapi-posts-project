...
@pytest.mark.parametrize("title, content, published", [
    ...
    ("Favorite pizza", "I love peeroni", False),
    ("Tallest skyscrapers", "Really big ones", True),
...
def test_create_post_default_published_true(authorized_client, test_user, test_posts):
    res = authorized_client.post("/posts/", json={
        "title": "arbitrary titles",
        "content": "ASDASDASDASDAS",
    })

    created_post = schemas.Post(**res.json())
    assert res.status_code == 201
    assert created_post.title == "arbitrary titles"
    assert created_post.content == "ASDASDASDASDAS"
...
def test_success_delete_post(client, authorized_client, test_posts):
    res = authorized_client.delete(f"/posts/{test_posts[0].id}")
    assert res.status_code == 204


def test_delete_post_non_exist(authorized_client, test_user, test_posts):
    res = authorized_client.delete(f"/posts/12321312321")
...
def test_update_post_non_exist(authorized_client, test_user, test_posts):
    ...
    res = authorized_client.put(f"/posts/12321312321", json=data)
